...
export function showPauseUI(uiElements: Element[], options: ShowPauseMenuOptions, callbacks: ShowPauseMenuCallbacks) {
  ...
  if (isChallengeLevel) {
    ...
    const yRow3 = 520;
    let x = xInitial;
    warpButton("X01", x + 0.00000, yRow1, 401, 'pauseButtonWarpX01');
    warpButton("X02", x += offset, yRow1, 402, 'pauseButtonWarpX02');
    warpButton("X03", x += offset, yRow1, 403, 'pauseButtonWarpX03');
    warpButton("X04", x += offset, yRow1, 404, 'pauseButtonWarpX04');
    warpButton("X05", x += offset, yRow1, 405, 'pauseButtonWarpX05');
    warpButton("X06", x += offset, yRow1, 406, 'pauseButtonWarpX06');
    x = xInitial;
    warpButton("X07", x + 0.00000, yRow2, 407, 'pauseButtonWarpX07');
    warpButton("X08", x += offset, yRow2, 408, 'pauseButtonWarpX08');
    warpButton("X09", x += offset, yRow2, 409, 'pauseButtonWarpX09');
    warpButton("X10", x += offset, yRow2, 410, 'pauseButtonWarpX10');
    warpButton("X11", x += offset, yRow2, 411, 'pauseButtonWarpX11');
    warpButton("X12", x += offset, yRow2, 412, 'pauseButtonWarpX12');
    x = xInitial + offset * 1.5;
    warpButton("X13", x + 0.00000, yRow3, 413, 'pauseButtonWarpX13');
    warpButton("X14", x += offset, yRow3, 414, 'pauseButtonWarpX14');
    warpButton("X15", x += offset, yRow3, 415, 'pauseButtonWarpX15');
  } else {
    const xInitial = 120;
    const offset = 60;
    const yRow1 = 440;
